...
// Builds the API-only Express app (no static file serving, no listen call).
// Shared between the standalone dev server (server/src/index.ts) and the
// Netlify serverless function (netlify/functions/api.ts).
export function createApp() {
  ...
  app.post("/api/clients/backfill", async (_req, res) => {
    const { backfillDefaultClient } = await import("./clients.js");
